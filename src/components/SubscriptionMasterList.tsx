      ...
      <div className="p-4 border-b border-border bg-muted/40">
        <div className="flex items-center justify-between gap-2 mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="px-2 py-0.5 rounded-full text-xs font-extrabold bg-indigo-500/10 text-indigo-500 border border-indigo-500/20">
          ...
          <button
            ...
            type="button"
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold transition-all shadow-md shadow-indigo-600/20 active:scale-95 cursor-pointer"
          >
        ...
        <div className="relative mb-3">
          ...
          <input
            ...
            className="w-full pl-10 pr-8 py-2 rounded-xl bg-background border border-border text-foreground placeholder-muted-foreground text-xs font-medium focus:outline-none focus:border-indigo-500 transition-colors"
          />
          {filters.search && (
            <button
              onClick={clearSearch}
        ...
        <div className="mt-2.5 relative">
          <select
            ...
            className="w-full appearance-none pl-3 pr-7 py-1.5 rounded-lg bg-background border border-border text-foreground text-[11px] font-semibold focus:outline-none focus:border-indigo-500 cursor-pointer"
          >
            {CATEGORIES.map((c) => (
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1.5 divide-y divide-transparent">
        {subscriptions.length === 0 ? (
          ...
          subscriptions.map((sub) => {
            ...
            const iconColor = sub.iconColorHex || sub.color || '#6366F1';
            ...
            return (
              <button
                ...
                className={`w-full text-left p-3 rounded-2xl transition-all duration-150 flex items-center justify-between gap-3 group relative overflow-hidden cursor-pointer ${
                  isSelected
                    ? 'bg-indigo-500/15 border border-indigo-500/50 shadow-md ring-1 ring-indigo-500/20'
              ...
              >
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  <ChevronRight className={`w-4 h-4 transition-transform ${
                    isSelected ? 'text-indigo-500 translate-x-0.5' : 'text-muted-foreground group-hover:text-foreground'
                  }`} />
                </div>
              </button>
            );
